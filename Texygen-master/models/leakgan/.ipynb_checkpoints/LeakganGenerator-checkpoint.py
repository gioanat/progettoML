...
class Generator(object):
    def __init__(self, sequence_length, num_classes, num_vocabulary,
        ...
        with tf.compat.v1.variable_scope('Worker'):
            self.g_embeddings = tf.Variable(tf.compat.v1.random_normal([self.num_vocabulary, self.emb_dim], stddev=0.1))
            ...
            self.W_workerOut_change = tf.Variable(tf.compat.v1.random_normal([self.num_vocabulary, self.goal_size], stddev=0.1))

            self.g_change = tf.Variable(tf.compat.v1.random_normal([self.goal_out_size, self.goal_size], stddev=0.1))
        ...
        def _g_recurrence(i, x_t, h_tm1, h_tm1_manager, gen_o, gen_x, goal, last_goal, real_goal, step_size,
            ...
            log_prob = tf.compat.v1.log(tf.nn.softmax(
        ...
        self.pretrain_goal_loss = -tf.reduce_sum(1 - tf.compat.v1.losses.cosine_distance(tf.nn.l2_normalize(self.sub_feature, 2),
        ...
        self.cross_entropy = tf.reduce_sum(
            self.g_predictions * tf.compat.v1.log(tf.clip_by_value(self.g_predictions, 1e-20, 1.0))) / (
                                 self.batch_size * self.sequence_length * self.num_vocabulary)

        self.pretrain_worker_loss = -tf.reduce_sum(
            tf.one_hot(tf.compat.v1.to_int32(tf.reshape(self.x, [-1])), self.num_vocabulary, 1.0, 0.0) * tf.compat.v1.log(
        ...
        self.goal_loss = -tf.reduce_sum(tf.multiply(self.reward, 1 - tf.compat.v1.losses.cosine_distance(
    ...
    def rollout(self, input_x, given_num):
        ...
        def _g_recurrence_2(i, x_t, gen_x, h_tm1, h_tm1_manager, last_goal, real_goal):
            ...
            log_prob = tf.compat.v1.log(tf.nn.softmax(x_logits))
            next_token = tf.cast(tf.reshape(tf.compat.v1.multinomial(log_prob, 1), [self.batch_size]), tf.int32)
            x_tp1 = tf.nn.embedding_lookup(self.g_embeddings, next_token)  # batch x emb_dim
            with tf.control_dependencies([cur_sen]):
    ...
    def create_Worker_recurrent_unit(self, params):
        with tf.compat.v1.variable_scope('Worker'):
            ...
            self.Wi_worker = tf.Variable(tf.compat.v1.random_normal([self.emb_dim, self.hidden_dim], stddev=0.1))
            self.Ui_worker = tf.Variable(tf.compat.v1.random_normal([self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bi_worker = tf.Variable(tf.compat.v1.random_normal([self.hidden_dim], stddev=0.1))

            self.Wf_worker = tf.Variable(tf.compat.v1.random_normal([self.emb_dim, self.hidden_dim], stddev=0.1))
            self.Uf_worker = tf.Variable(tf.compat.v1.random_normal([self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bf_worker = tf.Variable(tf.compat.v1.random_normal([self.hidden_dim], stddev=0.1))

            self.Wog_worker = tf.Variable(tf.compat.v1.random_normal([self.emb_dim, self.hidden_dim], stddev=0.1))
            self.Uog_worker = tf.Variable(tf.compat.v1.random_normal([self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bog_worker = tf.Variable(tf.compat.v1.random_normal([self.hidden_dim], stddev=0.1))

            self.Wc_worker = tf.Variable(tf.compat.v1.random_normal([self.emb_dim, self.hidden_dim], stddev=0.1))
            self.Uc_worker = tf.Variable(tf.compat.v1.random_normal([self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bc_worker = tf.Variable(tf.compat.v1.random_normal([self.hidden_dim], stddev=0.1))
            params.extend([
    ...
    def create_Worker_output_unit(self, params):
        with tf.compat.v1.variable_scope('Worker'):
            ...
            self.b_worker = tf.Variable(tf.compat.v1.random_normal([self.num_vocabulary * self.goal_size], stddev=0.1))
    ...
    def create_Manager_recurrent_unit(self, params):
        with tf.compat.v1.variable_scope('Manager'):
            # Weights and Bias for input and hidden tensor
            self.Wi = tf.Variable(tf.compat.v1.random_normal([self.num_filters_total, self.hidden_dim], stddev=0.1))
            self.Ui = tf.Variable(tf.compat.v1.random_normal([self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bi = tf.Variable(tf.compat.v1.random_normal([self.hidden_dim], stddev=0.1))

            self.Wf = tf.Variable(tf.compat.v1.random_normal([self.num_filters_total, self.hidden_dim], stddev=0.1))
            self.Uf = tf.Variable(tf.compat.v1.random_normal([self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bf = tf.Variable(tf.compat.v1.random_normal([self.hidden_dim], stddev=0.1))

            self.Wog = tf.Variable(tf.compat.v1.random_normal([self.num_filters_total, self.hidden_dim], stddev=0.1))
            self.Uog = tf.Variable(tf.compat.v1.random_normal([self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bog = tf.Variable(tf.compat.v1.random_normal([self.hidden_dim], stddev=0.1))

            self.Wc = tf.Variable(tf.compat.v1.random_normal([self.num_filters_total, self.hidden_dim], stddev=0.1))
            self.Uc = tf.Variable(tf.compat.v1.random_normal([self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bc = tf.Variable(tf.compat.v1.random_normal([self.hidden_dim], stddev=0.1))
            params.extend([
    ...
    def create_Manager_output_unit(self, params):
        with tf.compat.v1.variable_scope('Manager'):
            self.Wo = tf.Variable(tf.compat.v1.random_normal([self.hidden_dim, self.goal_out_size], stddev=0.1))
            self.bo = tf.Variable(tf.compat.v1.random_normal([self.goal_out_size], stddev=0.1))
