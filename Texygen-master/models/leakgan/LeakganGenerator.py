...
class Generator(object):
    def __init__(self, sequence_length, num_classes, num_vocabulary,
        ...
        self.epis = 0.65
        self.tem = 0.9
        # self.drop_out = tf.compat.v1.placeholder(tf.float32)
        ...
        self.cross_entropy = tf.reduce_sum(
            self.g_predictions * tf.math.log(tf.clip_by_value(self.g_predictions, 1e-20, 1.0))) / (
                                 self.batch_size * self.sequence_length * self.num_vocabulary)

        self.pretrain_worker_loss = -tf.reduce_sum(
            tf.one_hot(tf.cast(tf.reshape(self.x, [-1]), tf.int32), self.num_vocabulary, 1.0, 0.0) * tf.math.log(
