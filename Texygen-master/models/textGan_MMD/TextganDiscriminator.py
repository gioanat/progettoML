    ...
    def __init__(
        ...
        with tf.compat.v1.variable_scope('discriminator'):
            ...
            with tf.name_scope("output"):
                self.Wo = tf.Variable(tf.compat.v1.truncated_normal([num_filters_total, self.num_classes], stddev=0.1), name="W")
                self.bo = tf.Variable(tf.constant(0.1, shape=[self.num_classes]), name="b")

            # recon layer
            with tf.name_scope("recon"):
                self.Wzh = tf.Variable(tf.compat.v1.truncated_normal([num_filters_total, 1], stddev=0.1), name="Wz")
                self.bzh = tf.Variable(tf.compat.v1.constant(0.0, shape=[1]), name="bz")
            ...
            with tf.name_scope("loss"):
                ...
                gan_loss = tf.compat.v1.log(tf.norm(pos_score - pos_label, ord=1))
